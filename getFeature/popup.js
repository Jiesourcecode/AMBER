...
function addItems(time){
var length = window.localStorage.length + 1;
window.localStorage.setItem('Tasks'+length,time); 
}
...
var startTimeString = startTime.getHours()+':'+startTime.getMinutes()+':'+startTime.getSeconds()+':'+startTime.getMilliseconds();
document.getElementById('start').innerHTML = startTimeString;

var endTime = new Date(t.loadEventEnd);
endTimeString = endTime.getHours()+':'+endTime.getMinutes()+':'+endTime.getSeconds()+':'+endTime.getMilliseconds();
document.getElementById('end').innerHTML = endTimeString;

//addItems(startTimeString+'--'+endTime.getMinutes()+':'+endTime.getSeconds()+':'+endTime.getMilliseconds())
